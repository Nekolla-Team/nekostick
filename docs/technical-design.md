...
**PITR 恢复流程：** 节点拒绝接受低于当前已发布版本的快照（版本回退永不自动收敛）。数据库 PITR 回退到较早时间点后，**必须重启所有节点**——重启时节点的已发布状态为空，接受任意版本；否则节点持续拒绝回退后的配置版本，永久停留在内存中的旧快照。
...
**租约 fencing：** 续约得到确定的 `Conflict` / `NotFound`（所有权确认丢失）时，节点立即停止本地进程并撤回端点。数据库不可达或租约状态**不确定**期间不杀进程：租约本地仍有效时保留端点继续转发（与 4.3 的降级语义一致）；租约本地过期后端点随之撤回（过期租约不再可用于路由解析），该代次也不再自动续约——进程保留但不接受流量，恢复只能由请求或配置变更触发新的启动（冷启动新进程），旧进程在替换后随 drain 退出。同 nodeId 接管造成的双活窗口由 nodeId=0 致命退出和部署纪律（命名节点不得同时运行两份同 ID 进程）缓解。

**启动前置等待（Waiting）：** 服务缺少节点本地启动前置（典型：部署方扩展尚未把微服务二进制同步到本节点）时进入 `Waiting`，以退避自动重试且不消耗 restart burst 配额；前置满足后自动恢复，属主扩展也可经 `Supervisor.ResumeAsync`（API 1.3.3）主动恢复。`${HOST:VAR}` 占位符缺变量不属于等待场景：启动被立即拒绝并明确报错。
